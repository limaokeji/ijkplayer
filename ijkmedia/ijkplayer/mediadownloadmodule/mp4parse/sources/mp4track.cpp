	...
	bool* pIsSyncSample)
{
	...
	{
		throw new MP4Error("sample buffer is too small","MP4Track::ReadSample");
	}
...
u_int32_t MP4Track::GetSampleSize(MP4SampleId sampleId)
{
	...
	return m_bytesPerSample *  m_pStszSampleSizeProperty->GetValue(sampleId - 1);
}
...
u_int32_t MP4Track::GetSampleStscIndex(MP4SampleId sampleId)
{
	...
	for (stscIndex = 0; stscIndex < numStscs; stscIndex++) {
		u_int32_t chunck_sample_num = m_pStscFirstSampleProperty->GetValue(stscIndex);
		if (sampleId < chunck_sample_num) {
			ASSERT(stscIndex != 0);
			stscIndex -= 1;
			break;
		}
...
u_int64_t MP4Track::GetSampleFileOffset(MP4SampleId sampleId)
{
	...
	u_int32_t firstChunk = 	m_pStscFirstChunkProperty->GetValue(stscIndex);

	MP4SampleId firstSample = m_pStscFirstSampleProperty->GetValue(stscIndex);

	u_int32_t samplesPerChunk = m_pStscSamplesPerChunkProperty->GetValue(stscIndex);

	// chunkId tells which is the absolute chunk number that this sample
	// is stored in.
	MP4ChunkId chunkId = firstChunk + ((sampleId - firstSample) / samplesPerChunk);
	...
	MP4SampleId firstSampleInChunk = sampleId - ((sampleId - firstSample) % samplesPerChunk);
	...
	MP4Timestamp* pStartTime, MP4Duration* pDuration)
{
	...
	{
		u_int32_t sampleCount = m_pSttsSampleCountProperty->GetValue(sttsIndex);
		u_int32_t sampleDelta = m_pSttsSampleDeltaProperty->GetValue(sttsIndex);
	...
	throw new MP4Error("sample id out of range", "MP4Track::GetSampleTimes");
}

MP4SampleId MP4Track::GetSampleIdFromTime(MP4Timestamp when, bool wantSyncSample) 
{
	u_int32_t numStts = m_pSttsCountProperty->GetValue();
	MP4SampleId sid = 1;
	MP4Duration elapsed = 0;
...
u_int32_t MP4Track::GetSampleCttsIndex(MP4SampleId sampleId, MP4SampleId* pFirstSampleId)
{
...
u_int32_t  MP4Track::GetSyncSampleCount()
{
	if (m_pStssCountProperty == NULL) {
		return 0;
	}
	return m_pStssCountProperty->GetValue();
}

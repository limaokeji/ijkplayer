...
extern "C" MP4FileHandle MP4Read(char * fileName ,FILE * pFILE, u_int32_t verbosity)
{
	MP4File* pFile = NULL;
	try {
		pFile = new MP4File(verbosity);
		pFile->Read(fileName,pFILE);
		...
		u_int16_t modeSet)
{
	...
	{
		try {
			((MP4File*)hFile)->SetAmrModeSet(trackId, modeSet);
		}
		catch (MP4Error* e) {
			PRINT_ERROR(e);
			delete e;
				     ...
				     MP4TrackId trackId)
{
	if (MP4_IS_VALID_FILE_HANDLE(hFile)) 
	{
		try 
		{
		  return ((MP4File*)hFile)->GetAmrModeSet(trackId);
		}
		catch (MP4Error* e) 
		{
			PRINT_ERROR(e);
					       ...
					       uint16_t pictLen)
{
  if (MP4_IS_VALID_FILE_HANDLE(hFile)) 
  {
    try 
	{
      ...
      return pFile->AddH264PictureParameterSet(trackId,pPict,pictLen);
    }
    catch (MP4Error* e) 
	{
      PRINT_ERROR(e);
                                          ...
                                          )
{
  ...
  MP4SetTrackTimeScale(dstFile, dstTrackId,
		       MP4GetTrackTimeScale(srcFile, srcTrackId));
...
extern "C"  MP4SampleId MP4GetNextSyncSampleId(
	MP4FileHandle hFile,
	MP4TrackId trackId,
	MP4SampleId curSampleId)
{
	if (MP4_IS_VALID_FILE_HANDLE(hFile)) {
		try {
			return ((MP4File*)hFile)->MP4GetNextSyncSampleId(
				trackId, curSampleId);
		}
		catch (MP4Error* e) {
			PRINT_ERROR(e);
...
extern "C"  u_int32_t MP4GetSyncSampleCount(
	MP4FileHandle hFile,
	MP4TrackId trackId)
{
	if (MP4_IS_VALID_FILE_HANDLE(hFile)) {
		try {
			return ((MP4File*)hFile)->MP4GetSyncSampleCount(
				trackId);
		}
		catch (MP4Error* e) {
			PRINT_ERROR(e);
...
extern "C"  u_int64_t MP4GetSampleFileOffset(
	MP4FileHandle hFile,
	MP4TrackId trackId,
	MP4SampleId curSampleId)
{
	if (MP4_IS_VALID_FILE_HANDLE(hFile)) {
		try {
			return ((MP4File*)hFile)->MP4GetSampleFileOffset(
				trackId, curSampleId);
		}
		catch (MP4Error* e) {
			PRINT_ERROR(e);

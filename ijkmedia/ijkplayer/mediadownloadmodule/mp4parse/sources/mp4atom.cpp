...
MP4Atom* MP4Atom::ReadAtom(MP4File* pFile, MP4Atom* pParentAtom)
{
	...
	VERBOSE_READ(pFile->GetVerbosity(), printf("ReadAtom: pos = 0x"X64"\n", pos));

	u_int64_t dataSize = pFile->ReadUInt32();   //读块的长度

	char type[5];
	pFile->ReadBytes((u_int8_t*)&type[0], 4);   // 读块的类型  4个可读字符
	type[4] = '\0';
	...
	VERBOSE_READ(pFile->GetVerbosity(), printf("ReadAtom: type = \"%s\" data-size = "U64" (0x"X64") hdr %u\n", 	type, dataSize, dataSize, hdrSize));

	if (pos + hdrSize + dataSize > pParentAtom->GetEnd()) //错误的块内容
	{
...
void MP4Atom::Read()
{
	...
	{
		VERBOSE_READ(GetVerbosity(), printf("Warning: %s atom size "U64" is suspect\n",m_type, m_size));
	}
...
void MP4Atom::Skip()
{
	...
	{
		VERBOSE_READ(m_pFile->GetVerbosity(),printf("Skip: "U64" bytes\n", m_end - m_pFile->GetPosition()));
	}
...
void MP4Atom::ReadChildAtoms()
{
	...
	for (u_int64_t position = m_pFile->GetPosition();position < m_end;position = m_pFile->GetPosition()) 
	{
			...
			if (m_end - position < 2 * sizeof(uint32_t)) 
			{
				...
				if (this_is_udta &&	m_end - position == sizeof(uint32_t)) 
				{
	...
	for (u_int32_t i = 0; i < numAtomInfo; i++) {
		if (m_pChildAtomInfos[i]->m_mandatory && m_pChildAtomInfos[i]->m_count == 0) 
		{
				VERBOSE_READ(GetVerbosity(),
